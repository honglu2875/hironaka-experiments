...
def experiment(device_num, config):
    ...
    exp_cfg['host']['net_arch'] = [{'repeat': 2, 'net_arch': ['r256']}]  # Override with small network
    ...
    for i in range(20):
        trainer.train(10000, evaluation_interval=999, players=('host',))
        trainer.save(f"models/{trainer.version_string}_EXP1_{device_num}_ckpt_{i}.pt")

    print(trainer.evaluate_rho())
    
def main(config_path: str = 'small_scale_v0.yml'): 
